...
from preprocessing.utils import get_obj_name, load_split
...
avg_labeled_parts = { obj: round(num_labeled_parts[obj] / num_obj_instances[obj], 2) for obj in num_labeled_parts }
num_instances_with_affordance = stats['affordances']
frac_instances_with_affordance = { aff: round(count / total_obj_instances, 2) for aff, count in stats['affordances'].items() }
num_affordances = len([aff for aff, count in stats['affordances'].items() if count > 0])
...
summary = {
    'Selected object classes': class_names,
    'Selected affordances': affordances,
    'Number of matched affordances': num_affordances,
    'Coverage of selected affordances': num_affordances / len(affordances),
    'Number of matched classes': num_classes,
    'Coverage of selected classes': num_classes / len(class_names),
    'Number of object instances': total_obj_instances,
    'Number of object instances by class': sort_dict(num_obj_instances),
    'Number of labeled parts by class (total)': sort_dict(num_labeled_parts),
    'Number of labeled parts by class (avg per instance)': sort_dict(avg_labeled_parts),
    'Number of object classes containing affordance': sort_dict(num_class_with_affordance),
    'Number of instances containing affordance': sort_dict(num_instances_with_affordance),
    'Fraction of instances containing affordance': sort_dict(frac_instances_with_affordance)
}
